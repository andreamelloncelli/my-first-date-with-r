...
# Compute the summary (minimum, first quartile, median, mean, third quartile, maximum and standard deviation) of balance of the accounts  
bank %>% summarise(min=min(balance, na.rm = TRUE),
...
# Compute the summary (minimum, first quartile, median, mean, third quantile, maximum and standard deviation) of balance of the accounts by job 
bank %>% 
  group_by(job) %>%
  summarise(min=min(balance, na.rm = TRUE),
              first_q=quantile(balance, prob = 0.25, na.rm = TRUE),
              median=median(balance, na.rm = TRUE),
              mean=mean(balance, na.rm =TRUE),
              third_q=quantile(balance, prob = 0.75, na.rm = TRUE),
...
## ----multiple_op---------------------------------------------------------
# Compute the mean of balance of the accounts by job for people not older than 40 years and sort the result in ascending order  
bank %>%
  group_by(job) %>%
  filter(age < 40) %>%
  summarise(mean_balance = mean(balance, na.rm = TRUE)) %>%
